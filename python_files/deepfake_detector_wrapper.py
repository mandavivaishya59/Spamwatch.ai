import torch
from mukh.deepfake_detection.models.resnet_inception.resnet_inception_detector import ResNetInceptionDetector
...
class DeepfakeDetector:
    def __init__(self, model_name="resnet_inception", confidence_threshold=0.5, device=None):
        ...
        self.detector_classes = {
            "resnet_inception": ResNetInceptionDetector,
            # Add other models here if available, e.g.:
            # "efficientnet": EfficientNetDetector,
        }

        if model_name not in self.detector_classes:
            raise ValueError(f"Unsupported model_name: {model_name}")

        self.detector = self.detector_classes[model_name](
